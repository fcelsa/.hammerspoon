...
function MonthPrinter(anno, mese)
    ...
    local function get_day_of_week(dd, mm, yy)
        ...
        local val8 = dd + (mmx * 2) + math.floor(((mmx + 1) * 3) / 5) + yy + math.floor(yy / 4) - math.floor(yy / 100) +
            math.floor(yy / 400) + 2
    ...
    for i, v in ipairs(days_of_week_ordered) do
        if v.dayname == "Sab" then
            out = out ..
            hs.styledtext.ansi(fgMgn .. " Sab" .. ansiReset, { font = { name = "Monospac821 BT", size = 16 } })
        elseif v.dayname == "Dom" then
            out = out ..
            hs.styledtext.ansi(fgRed .. " Dom" .. ansiReset, { font = { name = "Monospac821 BT", size = 16 } })
        else
    ...
    while (d < month_days + 1) do
        ...
        if weekNumber == os.date("%V", os.time { year = oggi.year, month = oggi.month, day = oggi.day }) then
            out = out ..
                hs.styledtext.ansi(fgBlu .. getWeekNumber(d) .. "*" .. ansiReset,
                { font = { name = "Monospac821 BT", size = 16 } })
        else
            out = out ..
                hs.styledtext.ansi(fgBBlk .. getWeekNumber(d) .. "|" .. ansiReset,
                    { font = { name = "Monospac821 BT", size = 16 } })
        end
