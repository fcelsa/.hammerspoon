...
-- questo serve perché è un file esterno ad uno spoon (pseudo classe ?)
-- sarà chiamato, per esempio, come segue:
...
local function calc_easter(year)
    ...
    return string.format("%02d-%02d", day, month), string.format("%02d-%02d", mday, mmonth)
end
...
module.monthPrinter = function(anno, mese)
    local annoElab = anno
    local meseElab = mese
    ...
    -- inizo definizione dello stringone in ansi char da inviare come output
    local out = hs.styledtext.ansi("" .. ansiReset)
    local monthEsteso = os.date("%B", os.time { year = annoElab, month = meseElab, day = 1 })
    local currentDayExt = os.date("%A %d")
    local annoDiverso = ""
    if oggi.year ~= annoElab then
        annoDiverso = annoElab
    end

    if monthEsteso == os.date("%B", os.time { year = oggi.year, month = oggi.month, day = oggi.day }) and annoElab == oggi.year then
        out = out ..
        hs.styledtext.ansi(fgBlu .. "[ " .. currentDayExt .." ".. monthEsteso .. " ]" .. ansiReset .. "\n",
                { font = { name = fontName, size = fontSize } })
    else
        out = out ..
        hs.styledtext.ansi(monthEsteso .. "  " .. annoDiverso .. "\n", { font = { name = fontName, size = fontSize } })
    end

    out = out .."   "
    for i,v in ipairs(days_of_week_ordered) do
        if v.dayname == "Sab" then 
            out = out .. hs.styledtext.ansi(fgMgn .. " Sab" .. ansiReset, {font={name=fontName,size=fontSize}})
        elseif v.dayname == "Dom" then
            out = out .. hs.styledtext.ansi(fgRed .. " Dom" .. ansiReset, {font={name=fontName,size=fontSize}})
        else
            out = out .. hs.styledtext.ansi(" " .. v.dayname, {font={name=fontName,size=fontSize}})
        end
        if (day_week == v.daynum) then
            d = - i + 2
        end
    end
    out = out .. hs.styledtext.ansi("\n" .. ansiReset, {font={name=fontName,size=fontSize}})
    ...
    while (d < month_days + 1) do
        ...
        if weekNumber == os.date("%V", os.time { year = oggi.year, month = oggi.month, day = oggi.day }) and annoElab == oggi.year then
            out = out ..
            hs.styledtext.ansi(fgBlu .. getWeekNumber(d) .. "*" .. ansiReset, { font = { name = fontName, size = fontSize } })
        else
            out = out ..
            hs.styledtext.ansi(fgBBlk .. getWeekNumber(d) .. "|" .. ansiReset, { font = { name = fontName, size =
            fontSize } })
        end
        ...
        for i, v in ipairs(days_of_week) do
            if (d >= 1 and d <= month_days) then
                ...
                if d == dd and mm == oggi.month and annoElab == oggi.year then
